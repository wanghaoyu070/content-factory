...
describe('/api/wechat-articles-by-account', () => {
  ...
  it('returns 400 when accountName is missing', async () => {
    ...
    const request = new Request('http://localhost/api/wechat-articles-by-account', {
      method: 'POST',
  ...
  it('returns mock data when config is unavailable and fallback is enabled', async () => {
    ...
    const request = new Request('http://localhost/api/wechat-articles-by-account', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ accountName: '测试公众号', page: 1 }),
    });
    ...
    expect(body.accountInfo.name).toBe('测试公众号');
  });
  ...
  it('returns 504 when upstream times out and fallback is disabled', async () => {
    ...
    mockGetWechatArticleConfig.mockReturnValueOnce({
      endpoint: 'https://api.test.local/kw_search',
      apiKey: 'test-key',
    });
    mockFetchWithTimeout.mockRejectedValueOnce(new HttpTimeoutError('请求超时（>12000ms）'));
